...
public interface WalletV2JpaRepository extends JpaRepository<WalletV2Entity, String> {
    ...
    List<WalletV2Entity> findByAccountIdInAndType(
            @Param("accountIds") List<String> accountIds,
            @Param("type") WalletV2Type type
    );
    ...
    @Query("SELECT new org.marketplace_lea.common.dtos.WalletAccountV2DTO(w.id, w.investedCoinAmount, w.balance, w.account.id, w.account.login, w.numberAndOperator) FROM WalletV2 w WHERE w.walletType = :type AND w.investedCoinAmount > 0")
    Page<WalletAccountV2DTO> findAvailableConsomPointsForSale(Pageable pageable);

    @Query("SELECT new org.marketplace_lea.common.dtos.WalletAccountV2DTO(w.id, w.investedCoinAmount, w.balance, w.account.id, w.account.login, w.numberAndOperator) FROM WalletV2 w WHERE w.walletType = :type AND w.balance > 0")
    Page<WalletAccountV2DTO> findWalletsByBalancePositive(@Param("type") WalletV2Type type, Pageable pageable);
